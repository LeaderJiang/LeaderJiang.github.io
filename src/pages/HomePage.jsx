...
export default function HomePage() {
  return (
    <Stack spacing={3}>
      <Paper sx={{ p: { xs: 3, md: 4 }, borderRadius: 1.5, ...frameSx, ...cutCornerSx }}>
        ...
        <Grid container spacing={{ xs: 4, md: 5 }} alignItems="stretch">
          <Grid size={{ xs: 12, lg: 8 }}>
            <Stack spacing={2.5}>
              <Box sx={hudHeaderSx}>
                ...
                <Typography sx={hudCodeSx}>LIVE</Typography>
              </Box>
              ...
              <Grid container spacing={1.5} sx={{ pt: 1 }}>
                {[
                  ...
                  <Grid key={code} size={{ xs: 12, md: 4 }}>
                    <Box
                      sx={{
                        p: 2,
                        minHeight: '100%',
                        border: `1px solid ${alpha('#8ec5ff', 0.11)}`,
                        background: alpha('#081120', 0.26),
                      }}
                    >
                      <Typography sx={hudCodeSx}>{code}</Typography>
          ...
          <Grid size={{ xs: 12, lg: 4 }}>
            <Stack spacing={2}>
              ...
              <Paper sx={{ p: 2.5, borderRadius: 1.25, ...frameSx }}>
                <Box className="hud-corner hud-corner-tl" />
                <Box className="hud-corner hud-corner-br" />
                <Stack spacing={1.5}>
                  <Box sx={hudHeaderSx}>
                    <Typography sx={hudCodeSx}>STATUS</Typography>
                    <Typography variant="body1">COMMAND FEED</Typography>
                    <Typography sx={hudCodeSx}>SYNC</Typography>
                  </Box>
                  {[
                    'Portfolio routing active',
                    'GitHub Pages deployment stable',
                    'Multi-page HUD structure online',
                  ].map((item, index) => (
